...
def equirectangular_projection_batch(intrinsic, points_3d):
    ...
    Returns:
        projected_points (np.ndarray): An array of 2D points in equirectangular projection with shape (n, 2).
    """
...
def extract_colors_from_image(image, points_2d):
    """
    Extracts colors from the image at the 2D points.
    Args:
        image: The target image.
        points_2d: The 2D points to extract colors from the image.
    Returns:
        List of colors extracted from the image.
    """
    colors = []
...
if __name__ == "__main__":
    ...
    for i, ply_file in enumerate(ply_files_list):
        ...
        colors = extract_colors_from_image(original_image, points_2d)
        ...
        # white background
        plt.imshow(np.ones_like(original_image) * 255)
        map_colors = np.array(colors) / 255.0
        plt.scatter(points_2d[:, 0], points_2d[:, 1], s=0.3, c=map_colors, cmap='viridis', alpha=0.3)
        plt.axis("off")
        plt.show()

        # Save the ply file with colors
        ply_colors_path = ply_path.replace(".ply", "_colors.ply")
        # using o3d
        pcd_with_colors = o3d.geometry.PointCloud()
        pcd_with_colors.points = o3d.utility.Vector3dVector(valid_points)
        pcd_with_colors.colors = o3d.utility.Vector3dVector(map_colors)
        o3d.io.write_point_cloud(ply_colors_path, pcd_with_colors, write_ascii=True)
        print(f"Point cloud with colors saved to {ply_colors_path}")
